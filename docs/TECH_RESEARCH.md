...
| 抖音验证码 | 操作被拦截 | 中 | 每步检测验证码，三重提醒用户（Dock弹跳+通知+声音），无限等待手动处理 |
| 抖音风控检测 | 账号被限制或封禁 | 中 | 串行执行、随机间隔、`--disable-blink-features=AutomationControlled`、Chrome 始终可见窗口 |
| Chrome 进程管理 | 进程残留或端口冲突 | 低 | 启动前杀掉旧进程，动态端口 9222-9322 |
| Cookie 过期（执行中） | 登录失效 | 中 | 每步检测 URL 是否跳转到登录页，过期则跳过该账号所有任务；账号管理页提供「检查登录状态」按钮 |
| Chrome 下载失败 | 首次无法使用 | 中 | 3 次重试（间隔 3s），详细错误提示 |
